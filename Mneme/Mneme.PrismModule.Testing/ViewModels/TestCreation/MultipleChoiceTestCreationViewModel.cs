...
public class MultipleChoiceTestCreationViewModel : BindableBase, INavigationAware, IDialogAware
{
	...
	private void CreateTest()
	{
		...
		if (editMode)
		{
			...
			for (var i = 0; i < amountOfAnswers; i++)
			{
				test.Answers.Add(new TestMultipleChoice { Answer = Texts[i], IsCorrect = Checks[i], Test = test });
			}
		...
		} else
		{
			...
			for (var i = 0; i < amountOfAnswers; i++)
			{
				answers.Add(new TestMultipleChoice { Answer = Texts[i], IsCorrect = Checks[i], Test = test });
			}
	...
	{
		_ = parameters.TryGetValue("test", out TestMultipleChoices test);
